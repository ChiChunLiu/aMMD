...
##   --at:            specify where to add a branch; otherwise, generate all possible cases
...
def node_count(graph):
    tv1 = sum([v[1:] if v[0] == "root" else v[2:] if v[0] == "label" or v[0] == "edge" else v[1:4] for v in graph], [])
...
outFile = (inFile if p1 == "" else inFile + "." + p1)


###################################################################
## 2. Write down a new graph including an additional population  ##

rns = ["root", "label", "edge", "admix"]
