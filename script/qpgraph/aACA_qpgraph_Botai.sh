...
param_directory="/scratch/cliu7/ancient_ACA/output/qpgraph/Botai"
...
echo "indivname: DIR/${data_prefix}.grouped2.ind" >> ${param_file}
...
for b in ${basalEA}
do
	cp ${b}_5pops_Botai/Mbuti.MA-1.${b}.Ami.DevilsGate.Botai.2way.5.graph\
		Mbuti.MA-1.${b}.Ami.DevilsGate.Botai.graph
done
...
for b in ${basalEA}; do
    cd ${b}_6pops_Mixe
    graph1="Mbuti.MA-1.${b}.Ami.DevilsGate.Botai"
...
for b in ${basalEA}
do
        cp ${b}_6pops_Mixe/Mbuti.MA-1.${b}.Ami.DevilsGate.Botai.Mixe.2way.44.graph \
                Mbuti.MA-1.${b}.Ami.DevilsGate.Botai.Mixe.graph
done
...
for b in ${basalEA}; do
    ...
    graph1="Mbuti.MA-1.${b}.Ami.DevilsGate.Botai.Mixe"
...
for b in ${basalEA}
do
        cp ${b}_7pops_Upper_YR_LN/Mbuti.MA-1.${b}.Ami.DevilsGate.Botai.Mixe.Upper_YR_LN.2way.46.graph \
                Mbuti.MA-1.${b}.Ami.DevilsGate.Botai.Mixe.Upper_YR_LN.graph
done

groups="Tibetan_Shannan Kyang Lubrak Mebrak Rhirhi Samdzong Chokhopani Sherpa.LC UpperMustang.LC"
...
for b in ${basalEA}; do
    cd ${b}_8pops_Kyang
    graph1="Mbuti.MA-1.${b}.Ami.DevilsGate.Botai.Mixe.Upper_YR_LN"
